...
use crate::SimState;
use crate::selection::apply_camera_to_selection;

use super::util::{orbit_just_pressed, pan_just_pressed, apply_limits, calculate_from_translation_and_focus, update_orbit_transform, orbit_pressed, pan_pressed, orbit_just_released, lerp_and_snap_f32, lerp_and_snap_vec3};
        ...
        app.insert_resource(ActiveCameraData::default())
            .add_systems(
                ...
                (
                    ...
                    .after(apply_camera_to_selection),
            );
    }
}
    ...
    for (entity, mut pan_orbit, mut transform, mut projection) in orbit_cameras.iter_mut() {
        ...
        if !pan_orbit.initialized {
            ...
            let (alpha, beta, radius) =
                calculate_from_translation_and_focus(transform.translation, pan_orbit.focus);
            ...
            update_orbit_transform(alpha, beta, radius, pan_orbit.focus, &mut transform);
        ...
        if !pan_orbit.allow_upside_down {
            pan_orbit.target_beta =
                apply_limits(pan_orbit.target_beta, Some(PI / 2.0), Some(-PI / 2.0));
        }
