...
use bevy::prelude::{Resource, Query, Transform, Res, Quat, IntoSystemConfigs, Entity, in_state, Update, Without, With, Vec3, Gizmos, Color, Children, Mut, Visibility};
use bevy::text::Text;
use bevy::time::Time;
use bevy_mod_billboard::text::BillboardTextBounds;

use crate::body::{Planet, Moon, Star};
    ...
    for (children, p_transform, _, _, _) in planets.iter() {
        for child in children.iter() {
            if let Ok((_, mut transform, mut visible, _)) = billboards.get_mut(*child) {
                if radius > PLANET_VISIBILITY_THRESHOLD {
